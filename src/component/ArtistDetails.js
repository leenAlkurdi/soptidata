...
const ArtistDetails = ({ artistData, songs, allArtists, onBack }) => {  
  ...
  const calculateArtistPosition = (artistName) => {
    ...
    const artistPlaysMap = allArtists.map(artist => ({
      name: artist.master_metadata_album_artist_name,
      plays: artist.plays 
    }));
  
    artistPlaysMap.sort((a, b) => b.plays - a.plays);
  
    const position = artistPlaysMap.findIndex(artist => artist.name === artistName);
  ...
  return (  
    <div className="bg-white rounded-lg shadow-md m-4 p-4 w-full md:w-72">  
      ...
      <p className="text-gray-700">Total Time Spent: {artistData.timeSpent} days</p>  
      <p className="text-gray-700">Total Tracks: {artistData.totalTracks}</p>  
      <p className="text-gray-700">Most Listening Season: {artistData.season}</p>  
      <p className="text-gray-700">Position in Top 100 Artists: {artistPosition ? artistPosition : "Not in Top 100"}</p>

      <h3 className="text-lg font-semibold mt-4">Top 20 Unique Songs by Listening Time:</h3>  
      <ul className="list-disc pl-5 text-gray-700">  
        {uniqueTopSongs.length > 0 ? (
          uniqueTopSongs.map((song, index) => (
            <li key={index}>
              {song.master_metadata_track_name} - {Math.round(song.ms_played / 60000)} min
            </li>
          ))
        ) : (
